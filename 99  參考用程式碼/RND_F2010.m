...
Target_TTM = Data(1, Index_TTM);                                           % Update: Target_TTM

%% Data Filtering
% Bid >= 0.5
Index = find(Data(:, Index_OP_Bid) >= 0.5);
Data = Data(Index, :);                                                     % Update: Data
clear Index
    
% Out-of-the-Money (OTM) and Around At-the-Money (ATM) Options
Index = find((Data(:, Index_CPFlag)==1) & ...
             (Data(:, Index_K) >= (Data(:, Index_S) - BP)) & ...
             (Data(:, Index_K) <= (Data(:, Index_S) + BP)));
...
Data_DY = load(['E:\WRDS Data\' FileName]);
clear FileName
...
Data_RF = load(['E:\WRDS Data\' FileName]);
clear FileName
...
%% Calculation of Option-Implied Volatility (Using the Black-Scholes Formula)
% Data(:, Index_IV) = nan;                                                   % Update: Data (IV)
% 
...
% 
% Data(:, Index_IV) = blsimpv(Data(:, Index_S), Data(:, Index_K), Data(:, Index_RF), ...
%                             Data(:, Index_TTM) / 365, ...
%                             0.5 * (Data(:, Index_OP_Bid) + Data(:, Index_OP_Ask)), ...
%                             [], Data(:, Index_DY), [], Type);              % Update: Data (IV)              
% clear Type         

%% Blend Implied Volatilitys Around the At-the-Money (ATM) Level
Index = find((Data(:, Index_CPFlag)==31) | ...
             (Data(:, Index_CPFlag)==32));         
[AllK, ~, Index_AllK] = unique(Data(Index, Index_K));

for i = 1:length(AllK)
    Data_BP = Data(Index(Index_AllK==i), :);
...
% Smooth_AllK = max(K_Low, 5):0.05:(K_High - rem(K_High, 5));
clear K_Low K_High
